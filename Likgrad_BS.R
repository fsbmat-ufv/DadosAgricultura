loglik_BS <- function(par){
  n <- length(YO)
  NXS <- dim(model.matrix(~XS))[2]#Número de colunas da matriz de seleção
  NXO <- dim(model.matrix(~XO))[2]#Número de colunas da matriz de regressão primária
  ## parameter indices
  igamma <- 1:NXS
...
gradlik_BS <- function(par) {
 ...
 # if( ( rho < -1) || ( rho > 1)) return(matrix(NA, nObs, nParam))
  ...
  term8 <- ((phi2/2)^(1/2))*(((phi2+1)/(phi2*XS0.g))^(1/2)-((phi2*XS0.g)/(phi2+1))^(1/2))
  FT2 <- pnorm(term8, log.p = TRUE)
  term9 <-  ((-1/2)*(((YO1*(phi1+1))/(phi1*(XO1.b)))^(1/2)+((phi1*XO1.b)/(YO1*(phi1+1)))^(1/2)))*model.matrix(~XO1) #Derivada de term0 em relação a beta
  term10 <-           (1/2)*(((XO1.b*phi1)/((YO1^3)*(phi1+1)))^(1/2)-((phi1+1)/(YO1*phi1*XO1.b))^(1/2))*model.matrix(~XO1) #Derivada de term2 em relação a beta
  term11 <- ((-1/2)*(sqrt(phi2/2))*(((phi2+1)/(phi2*(XS0.g)))^(1/2)+((phi2*XS0.g)/((phi2+1)))^(1/2)))*model.matrix(~XS0) #Derivada de term8 em relação a gamma
  term12 <- (-1/2)*((YO1/((phi1^3)*XO1.b*(phi1+1)))^(1/2)+(XO1.b/(YO1*((phi1+1)^3)*phi1))^(1/2)) #Derivada de term0 em relação a phi1
  term13 <- (1/2)*((XO1.b/(phi1*((phi1+1)^3)*(YO1^3)))^(1/2)-(1/((phi1^3)*(phi1+1)*XO1.b*YO1))^(1/2))#Derivada de term2 em relação a phi1
  ...
  df4g <- lambda_I*((1/4)*(((XS1.g+2)/sqrt(XS1.g*(1-rho^(2))))))*model.matrix(~XS1) #Derivada de f4 em relação a gamma
