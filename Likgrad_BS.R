          ...
          start = NULL) 
{
  ...
  loglik_BS <- function(par) {
    ...
    ll <- sum(log(term2) + log(term1) + log(term3) + term7) + 
      sum(FT2)
    return(sum(ll))
  }
  gradlik_BS <- function(par) {
    ...
    term8 <- ((phi2/2)^(1/2)) * (((phi2 + 1)/(phi2 * XS0.g))^(1/2) - 
                                   ((phi2 * XS0.g)/(phi2 + 1))^(1/2))
    FT2 <- pnorm(term8, log.p = TRUE)
    term9 <- ((-1/2) * (((YO1 * (phi1 + 1))/(phi1 * (XO1.b)))^(1/2) + 
                          ((phi1 * XO1.b)/(YO1 * (phi1 + 1)))^(1/2))) * (XO1)
    term10 <- (1/2) * (((XO1.b * phi1)/((YO1^3) * (phi1 + 
                                                     1)))^(1/2) - ((phi1 + 1)/(YO1 * phi1 * XO1.b))^(1/2)) * 
      (XO1)
    term11 <- ((-1/2) * (sqrt(phi2/2)) * (((phi2 + 1)/(phi2 * 
                                                         (XS0.g)))^(1/2) + ((phi2 * XS0.g)/((phi2 + 1)))^(1/2))) * 
      (XS0)
    term12 <- (-1/2) * ((YO1/((phi1^3) * XO1.b * (phi1 + 
                                                    1)))^(1/2) + (XO1.b/(YO1 * ((phi1 + 1)^3) * phi1))^(1/2))
    term13 <- (1/2) * ((XO1.b/(phi1 * ((phi1 + 1)^3) * (YO1^3)))^(1/2) - 
                         (1/((phi1^3) * (phi1 + 1) * XO1.b * YO1))^(1/2))
    term14 <- rho/(2 * ((2 * phi1 * (1 - rho^2))^(1/2)))
    term15 <- (rho/(1 - rho^2)) * term4
    term16 <- (1/(1 - rho^2)) * ((phi1/(2 * (1 - rho^2)))^(1/2))
    lambda_I <- exp(dnorm(integrand, log = TRUE) - pnorm(integrand, 
                                                         log.p = TRUE))
    lambda_T8 <- exp(dnorm(term8, log = TRUE) - pnorm(term8, 
                                                      log.p = TRUE))
    df1b <- (-phi1/2) * term0 * term9
    df2b <- (term10/term2)
    df4b <- lambda_I * term9 * term6
    df4g <- lambda_I * ((1/4) * (((XS1.g + 2)/sqrt(XS1.g * 
                                                     (1 - rho^(2)))))) * (XS1)
    ...
    df4rho <- lambda_I * (term5 * term4 * (rho/(1 - rho^2)) + 
                            (term0 * term6/(rho * (1 - rho^2))))
    gradient <- matrix(0, nObs, nParam)
    gradient[YS == 0, igamma] <- w0 * df5g
    gradient[YS == 1, igamma] <- w1 * (df4g)
    gradient[YS == 1, ibeta] <- w1 * (df1b + df2b + df4b)
    gradient[YS == 1, iphi1] <- w1 * (df1phi1 + df2phi1 + 
                                        df3phi1 + df4phi1)
    gradient[YS == 1, irho] <- w1 * (df4rho)
    return(colSums(gradient))
  }
  theta_BS <- optim(start, loglik_BS, gradlik_BS, method = "BFGS", 
  ...
  a16 <- -2 * a2 + a9 * log(a8)
  cl <- class(theta_BS)
  result <- list(coefficients = a1, value = a2, loglik = -a2, 
                 counts = a3, hessian = a4, fisher_infoBS = a5, prop_sigmaBS = a6, 
                 level = a7, nObs = a8, nParam = a9, N0 = a10, N1 = a11, 
                 NXS = a12, NXO = a13, df = a14, aic = a15, bic = a16, 
                 initial.value = a)
